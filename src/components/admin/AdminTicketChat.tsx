    ...
    <div className="flex flex-col rounded-2xl border border-ink-100 bg-white">
      <div className="max-h-[26rem] space-y-3 overflow-y-auto p-5">
        {messages.length === 0 && (
          <p className="py-8 text-center text-sm text-ink-400">
            Няма съобщения. Отговорете на клиента.
          </p>
        )}
        {messages.map((m) => {
          ...
          return (
            ...
            >
              ...
              >
                {isStaff && (
                  <div className="mb-0.5 text-[0.65rem] font-bold uppercase tracking-wider text-white/80">
                    Вие (екип)
                  </div>
                )}
                <p className="whitespace-pre-wrap">{m.body}</p>
                <div className={cn("mt-1 text-[0.65rem]", isStaff ? "text-white/70" : "text-ink-400")}>
      ...
      <form onSubmit={send} className="flex items-center gap-2 border-t border-ink-100 p-3">
        <input
          ...
          placeholder="Отговорете на клиента..."
        ...
        <button
          ...
          className="flex h-10 w-10 flex-none items-center justify-center rounded-full bg-brand-500 text-white transition-colors hover:bg-brand-600 disabled:opacity-50"
          aria-label="Изпрати"
        >
